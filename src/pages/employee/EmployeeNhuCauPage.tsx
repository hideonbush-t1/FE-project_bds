...
export function EmployeeNhuCauPage() {
  ...
  // 1. Khai báo thông tin nhân viên đang đăng nhập
    ...
    { title: 'HÀNH ĐỘNG', key: 'action', render: (_: any, bdsRecord: any) => (
      <button 
        ...
        onClick={() => {
          ...
          // Điều hướng về trang giao dịch của employee
        ...
        <Modal title={<div style={{ color: GOLD_COLOR, fontSize: '18px', paddingBottom: '10px' }}>{editingId ? 'CẬP NHẬT NHU CẦU' : 'TẠO NHU CẦU'}</div>} open={isFormVisible} onCancel={() => setIsFormVisible(false)} footer={null} width={750}>
          <Form form={form} layout="vertical" onFinish={handleFinishForm}>
            <Row gutter={[24, 16]}>
              ...
              <Col span={24}><Form.Item name="nhanVienHienThi" label="Nhân viên tạo"><Input disabled style={{ backgroundColor: '#13141f', color: GOLD_COLOR, fontWeight: 'bold' }} /></Form.Item></Col>
