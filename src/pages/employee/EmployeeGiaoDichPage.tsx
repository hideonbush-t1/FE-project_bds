...
export function EmployeeGiaoDichPage() {
  ...
  // Xử lý dữ liệu truyền từ trang khác sang (nếu có)
  useEffect(() => {
  ...
  // Mở form do điều hướng từ trang khác
  useEffect(() => {
...
batDongSanId: pendingInitData.batDongSanId, 
  ...
  const handleOpenEdit = (record: any) => {
    ...
    form.setFieldsValue({
      ...
      nhanVienHienThi: record.nhanVienId, // Có thể dùng get name nếu cần
  ...
  const handleFinishForm = async (values: any) => {
    try {
      const payload: any = {
        nhanVienId: values.nhanVienId || currentNhanVien.id, // Ưu tiên ID đã lưu nếu là Sửa
      ...
      <div className="bds-container">
        ...
        <div className="bds-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '15px' }}>
          ...
          <div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}>
            <input 
              type="text" placeholder="Tìm Mã GD, Mã BĐS, Tên KH..." value={searchText}
...
<td style={{ color: GOLD_COLOR, fontWeight: 'bold' }}>{gd.batDongSanId}</td>
        ...
        <Modal title={<div style={{ color: GOLD_COLOR, fontSize: '18px', paddingBottom: '10px' }}>{editingId ? 'CẬP NHẬT GIAO DỊCH' : 'TẠO GIAO DỊCH MỚI'}</div>} open={isFormVisible} onCancel={() => setIsFormVisible(false)} footer={null} width={800}>
          <Form form={form} layout="vertical" onFinish={handleFinishForm}>
            <Row gutter={[24, 16]}>
              <Col xs={24} md={12}><Form.Item name="nhanVienHienThi" label="Nhân viên tạo/chốt"><Input disabled /></Form.Item></Col>
{/* Lưu chìm ID thật nếu cần */}
